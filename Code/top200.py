...
class CpEvent:
    ...
    def OnReceived(self):
        ...
        if (exFlag == ord('1')):  # 동시호가 시간 (예상체결)
            ws.append(["실시간(예상체결)", name, timess, "*", cprice,
                       "대비", diff, "체결량", cVol, "거래량", vol])
        elif (exFlag == ord('2')):  # 장중(체결)
            ws.append(["실시간(장중 체결)", name, timess, cprice,
                       "대비", diff, "체결량", cVol, "거래량", vol])

        wb.save("2022-02-09-top200.xlsx")
...
class CpMarketEye:
    def Request(self, codes, rqField):
        ...
        for i in range(cnt):
            ...
            ws.append([rpCode, rpName, rpTime,
                      rpDiffFlag, rpDiff, rpCur, rpVol])
        wb.save("2022-02-09-top200.xlsx")
        return True
...
class MyWindow(QMainWindow):
    ...
    def btnStart_clicked(self):
        ...
        ws.append([cnt, "종목 실시간 현재가 요청 시작"])
        wb.save("2022-02-09-top200.xlsx")
        self.isSB = True
